...
GO
ALTER TRIGGER [dbo].[trg_vrs_Operarios_puntoservicio]
...
AS
	...
	BEGIN TRY
		...
		DECLARE @dp0_estado nvarchar ;
		DECLARE @dp0_fechaFin date;
		DECLARE @hrm3_id int;
		DECLARE @hrm3_frecuencia nvarchar ;
		...
		DECLARE @hrm1_id int;
		DECLARE @hrm1_frecuencia nvarchar ;
		...
		DECLARE @hrm0_frecuencia nvarchar ;
		...
		OPEN cursorIt
		FETCH NEXT FROM cursorIt INTO @p_id,@p_CodPuntoServicio,@p_NombrePServicio,@p_DireccionContrato,@p_Barrios,@p_Contacto,@p_MailContacto,@p_TelefonoContacto,@p_Coordenadas,@p_Ciudad_id,@p_Cliente_id,@p_NumeroMarcador,@tmp_fechaIncio,@tmp_fechaFin,@tmp_vregistro
		WHILE @@FETCH_STATUS = 0
		BEGIN
			DECLARE @fechaCambio datetime;

			SET @fechaCambio=CURRENT_TIMESTAMP;
			IF (COLUMNS_UPDATED() & 8192)=8192
			BEGIN
			...
			BEGIN
				...
				update dbo.Operarios_puntoservicio set vfechaFin=@fechaCambio where id=@p_id;
					...
					DECLARE cursorSc0 CURSOR FOR SELECT * FROM dbo.Operarios_relevamientocab where puntoServicio_id=@p_id
					OPEN cursorSc0
					...
					DEALLOCATE cursorSc0
						...
						DECLARE cursorHrm0_3 CURSOR FOR SELECT * FROM dbo.Operarios_relevamientoesp where relevamientoCab_id=@dp0_id
						OPEN cursorHrm0_3
						...
						DECLARE cursorHrm0_2 CURSOR FOR SELECT * FROM dbo.Operarios_relevamientodet where relevamientoCab_id=@dp0_id
						OPEN cursorHrm0_2
						...
						DECLARE cursorHrm0_1 CURSOR FOR SELECT * FROM dbo.Operarios_relevamientocupohoras where
						...
						DECLARE cursorHrm0_0 CURSOR FOR SELECT * FROM dbo.Operarios_relevamientomensualeros where relevamientoCab_id=@dp0_id
						OPEN cursorHrm0_0
					...
					/* 1.B - Asiginacion */
					DECLARE cursorSc1 CURSOR FOR SELECT * FROM dbo.Operarios_asignacioncab where puntoServicio_id=@p_id
					OPEN cursorSc1
					...
					DEALLOCATE cursorSc1
							...
							DECLARE cursorHrm1_0 CURSOR FOR SELECT * FROM dbo.Operarios_asignaciondet where asignacionCab_id=@dp1_id
							OPEN cursorHrm1_0
							FETCH NEXT FROM cursorHrm1_0 INTO @hrm0_id,@hrm0_lunEnt,@hrm0_lunSal,@hrm0_marEnt,@hrm0_marSal,@hrm0_mieEnt,@hrm0_mieSal,@hrm0_jueE

							@hrm0_fechaInicio,@hrm0_totalHoras,@hrm0_fechaFin,@hrm0_supervisor,@hrm0_perfil_id,@tmp_fechaIncio,@tmp_fechaFin,@tmp_vregistro
					...
					/* 1.C - Planificacion*/
					DECLARE cursorSc2 CURSOR FOR SELECT * FROM dbo.Operarios_planificacioncab where puntoServicio_id=@p_id
					OPEN cursorSc2
					...
					print '55';
							...
							DECLARE cursorHrm2_1 CURSOR FOR SELECT * FROM dbo.Operarios_planificacionope where planificacionCab_id=@dp2_id
							OPEN cursorHrm2_1
							...
							DECLARE cursorHrm2_0 CURSOR FOR SELECT * FROM dbo.Operarios_planificacionesp where planificacionCab_id=@dp2_id
							OPEN cursorHrm2_0
...
ALTER TRIGGER [dbo].[trg_vrs_Operarios_relevamientocab]
...
AS
	...
	BEGIN TRY
		...
		BEGIN
			...
			OPEN cursorDt
			FETCH NEXT FROM cursorDt INTO @dpp0_id,@dpp0_fecha,@dpp0_cantidad,@dpp0_puntoServicio_id,@dpp0_cantidadHrTotal,@dpp0_cantidadHrEsp,@dpp0_fechaInicio,@dpp0_usuario_id,@dpp0_tipoSalario_id,@dpp0_comentario,@dpp0_cantAprendices,@dpp0_estado,@dpp0_fechaFin,@tmp_fechaIncio,@tmp_fechaFin,@tmp1_vregistro
			WHILE @@FETCH_STATUS = 0
			BEGIN
				DECLARE @fechaCambio datetime;
				SET @fechaCambio=CURRENT_TIMESTAMP;
...
ALTER TRIGGER [dbo].[trg_vrs_Operarios_relevamientodet]
...
ALTER TRIGGER [dbo].[trg_vrs_Operarios_relevamientoesp]
...
ALTER TRIGGER [dbo].[trg_vrs_ Operarios_relevamientocupohoras]
...
ALTER TRIGGER [dbo].[trg_vrs_Operarios_relevamientomensualeros]
...
ALTER TRIGGER [dbo].[trg_vrs_Operarios_planificacioncab]
...
ALTER TRIGGER [dbo].[trg_vrs_Operarios_planificacionope]
...
ALTER TRIGGER [dbo].[trg_vrs_Operarios_asignacioncab]
...
ALTER TRIGGER [dbo].[trg_vrs_Operarios_asignaciondet]
...
SET QUOTED_IDENTIFIER ON
GO
ALTER TRIGGER [dbo].[trg_vrs_Operarios_asigfiscalpuntoservicio]
...
SET QUOTED_IDENTIFIER ON
GO
ALTER TRIGGER [dbo].[trg_vrs_Operarios_asigjefefiscal]
...
/****** Object:  Index [UQ__Operario__FFF13D136436E2D7]    Script Date: 3/7/2019 17:13:44 ******/
ALTER TABLE [dbo].[Operarios_asigfiscalpuntoservicio] ADD UNIQUE NONCLUSTERED 
(
...
/****** Object:  Index [UQ__Operario__0437C365E7DEB8CE]    Script Date: 3/7/2019 17:14:15 ******/
ALTER TABLE [dbo].[Operarios_asigjefefiscal] ADD UNIQUE NONCLUSTERED 
(
	[userFiscal_id] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, SORT_IN_TEMPDB = OFF, IGNORE_DUP_KEY = OFF, ONLINE = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]
GO
...
DELETE FROM [dbo].[Operarios_asignaciondet] WHERE id != 157
DELETE FROM [dbo].[Operarios_planificacionesp] WHERE id != 4
DELETE FROM [dbo].[Operarios_planificacionope] WHERE id != 9
DELETE FROM [dbo].[Operarios_relevamientocupohoras] WHERE id != 5
DELETE FROM [dbo].[Operarios_relevamientodet] WHERE id != 1030
DELETE FROM [dbo].[Operarios_relevamientoesp] WHERE id != 4
DELETE FROM [dbo].[Operarios_relevamientomensualeros] WHERE id != 8

DELETE FROM [dbo].[Operarios_planificacioncab] WHERE id != 10
DELETE FROM [dbo].[Operarios_asignacioncab] WHERE id != 102
DELETE FROM [dbo].[Operarios_relevamientocab] WHERE id != 41
DELETE FROM [dbo].[Operarios_puntoservicio] WHERE id != 1
GO
